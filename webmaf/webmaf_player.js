...
// Playlist data:
// "films" below is the playlist and it requires 1 entry per video the parameters are:
//   1. Video path relative to the video_path_URL below
//   2. Licence URL, if required (primarily for use with PlayReady protection
//   3. Custom data, again for use with PlayReady protection
//   4. Video resume time in seconds from the start of the asset   0=from start (negative values indicate to choose a random start point -value seconds into the asset)
//   5. Video playtime before automatically loading the next asset
//   6. action to perform on the URL:
//        action="video", =play video
//        action="HTTPGET", just do a get & discard the results
//   7. Video content type, if this cannot be inferred from the URL valid types are listed below
...
// specify a URL as a path prefix for the videos in the playlist, must have trailing /
var video_path_URL="https://10fad4b4a7894c10a47117386cbbce08.dlvr1.net/FunimationStoreFront/";

var films=[  
  ["1709058/English/1709058_English_31294a18-d906-e811-8175-020165574d09.m3u8?Policy=eyJTdGF0ZW1lbnQiOlt7IlJlc291cmNlIjoiaHR0cHM6Ly9kMTMyZnVtaTZkaTF3YS5jbG91ZGZyb250Lm5ldC8qIiwiQ29uZGl0aW9uIjp7IkRhdGVMZXNzVGhhbiI6eyJBV1M6RXBvY2hUaW1lIjoxNTI2MzQ1OTQzfX19XX0_&Signature=dmw1IQJUEqBbvWhs9tH8gYKumk2Urk1nabEzc5VkwfmqitTkxYJqeJRZ-s7bLq7e6gH7lkHou7~q~9up2W3o0fKwYv2Rd37P7uNr1pGyjysOcuCavMLkvBub8unmxrrgoA897Z5PzKhYd-DrylOAUhYFD4IOEq5TlOdTdDvYxp0wT3mznHj3cknecoWeWUDW6XF8-KBF4wwgcipJ3eiwLAwa0OrOWOSoA6gIIVjsE7u9lpFaK2p2Vp1l6s2-Eyaamnqpt~37X51~YdM08ARpnvOh2F0eWAtaWzj9Ak1-S1QbfGx~tgqdhwqvH4jX4mE656v12FnLDsoZihjeogr6kQ__&Key-Pair-Id=DLVR"   ,"","",0  ,0    ,"video"   ,video_type_HLS ],
  ["1709060/English/1709060_English_90f7431e-d906-e811-8175-020165574d09.m3u8?Policy=eyJTdGF0ZW1lbnQiOlt7IlJlc291cmNlIjoiaHR0cHM6Ly9kMTMyZnVtaTZkaTF3YS5jbG91ZGZyb250Lm5ldC8qIiwiQ29uZGl0aW9uIjp7IkRhdGVMZXNzVGhhbiI6eyJBV1M6RXBvY2hUaW1lIjoxNTI2MzQ2MTY0fX19XX0_&Signature=l5hf-CGL4HsrYhSx3twhql1alkn-bdsJ6MGpeYXQ3hMenG23HmjYwhMz3h618jnG5CcAA-hhiDyMLORzirk0G9upDm7gCouhpef8rAei2N1lByUJ5Rky9WMB2y89ZHeZ8UCYdPCV1iKC4TqQ3aDUqIXjxmN3ztvEYIIFuTmRH3rnukI6DD8h5Rw9yD1PrUSgt5wslmXW4VyyuHxyo1T14Im78oQLa0B7lYDY1qYmmqtXD0~YHUBhKHaabVQkPZJWG0xXRj0YFYCDOKJysjUlMB6Gvs9NHOGekdUV-vHy6lgu3frz0~-KQj1K0NoF3y30myLJORx8d5Xd0qGf~hqowg__&Key-Pair-Id=DLVR"   ,"","",0 ,0   ,"video"   ,video_type_HLS ],
]
...
function webmaf_set_audio_track_with_workaround_for_fastforwarding(track_code) {
  ...
  video_API_load(video_path_URL+films[film_pos][0], films[film_pos][1], films[film_pos][2]);
...
function play_prev(){
  film_pos--;
  if (film_pos<0) film_pos=films.length;
  film_pos--;
  if (film_pos<0) film_pos=films.length;
  play_next();
}
...
function play_next() {
  ...
  switch (films[film_pos][5]) {
    case "video":
      ...
      current_time = -1;
      total_time = -1;
      video_API_load(video_path_URL+films[film_pos][0], films[film_pos][1], films[film_pos][2], films[film_pos][6]);
      next_movie_resume_time = films[film_pos][3];
      tick_time_to_play_next = films[film_pos][4];
      //              set_playtime(next_movie_resume_time);
      ...
      film_pos++;
      if (film_pos >= films.length) film_pos = 0;
      break;
    case "HTTPGET":
      var URL = films[film_pos][0];
