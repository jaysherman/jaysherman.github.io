...
/**
 * Prevent player from playing high bitrate streams.
 *
 * @param {number} streamIndex The stream index.
 * @param {number} qualityLevel The default quality level
 * @private
 */
...
sampleplayer.CastPlayer.prototype.loadVideo_ = function(info) {
  ...
  if (!protocolFunc) {
    ...
    if (!this.preloadPlayer_ || (this.preloadPlayer_.getHost &&
      ...
      //added by Jay Sherman - Sony DADC
      //override the quality level to prevent high bitrate streams
      host.getQualityLevel = function(streamIndex, qualityLevel) {
        var level = self.throttleQuality_(streamIndex,qualityLevel);
        return level;
      };
      //
